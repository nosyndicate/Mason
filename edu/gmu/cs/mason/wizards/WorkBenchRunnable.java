...
public class WorkBenchRunnable implements IRunnableWithProgress, IThreadListener {
	 ...
	 */
	public WorkBenchRunnable(IWorkspaceRunnable runnable) {
	 ...
	 */
	public WorkBenchRunnable(IWorkspaceRunnable runnable, ISchedulingRule rule) {
	 ...
	 *            thread
	 */
	public WorkBenchRunnable(IWorkspaceRunnable runnable, ISchedulingRule rule, boolean transfer) {
		fWorkspaceRunnable= runnable;
		fRule= rule;
		fTransfer= transfer;
	}
	...
	public void runAsUserJob(String name, final Object jobFamiliy) {
		Job job= new Job(name) {
			...
			protected IStatus run(IProgressMonitor monitor) {
				try {
					WorkBenchRunnable.this.run(monitor);
				} catch (InvocationTargetException e) {
					Throwable cause= e.getCause();
					if (cause instanceof CoreException) {
						return ((CoreException) cause).getStatus();
					} else {
						return new Status(Status.ERROR, Activator.PLUGIN_ID, "workbenchrunnable");
					}
